...
public interface SubastaRepository extends JpaRepository<Subasta, Long> {
    ...
    @Query("SELECT s FROM Subasta s WHERE subastaNormal = TRUE AND s.precioFinal >= :minPrice AND s.precioFinal <= :maxPrice AND s.categoria in :categorias ORDER BY CASE WHEN :dateOrder = 'asc' THEN s.fechaFinal END ASC, CASE WHEN :dateOrder = 'desc' THEN s.fechaFinal END DESC, s.subastaID ASC")
